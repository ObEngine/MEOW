...
////////////////////////////////////////////////////////////
#include <map>
#include <meow/Graphics/Export.hpp>
#include <meow/Graphics/Glsl.hpp>
#include <meow/System/NonCopyable.hpp>
#include <meow/System/Vector2.hpp>
#include <meow/System/Vector3.hpp>
#include <meow/Window/GlResource.hpp>
#include <string>
...
public:
  ...
  ///
  /// This overload can also be called with meow::Color objects
  /// that are converted to meow::Glsl::Vec4.
  ///
  /// It is important to note that the components of the color are
  /// normalized before being passed to the shader. Therefore,
  /// they are converted from range [0 .. 255] to range [0 .. 1].
  /// For example, a meow::Color(255, 127, 0, 255) will be transformed
  ...
  ///
  /// This overload can also be called with meow::Color objects
  /// that are converted to meow::Glsl::Ivec4.
  ///
  /// If color conversions are used, the ivec4 uniform in GLSL
  /// will hold the same values as the original meow::Color
  /// instance. For example, meow::Color(255, 127, 0, 255) is
  ...
  /// meow::Shader::CurrentTexture:
  /// \code
  /// shader.setUniform("the_texture", meow::Shader::CurrentTexture).
  /// \endcode
  ///
  ...
  /// \endcode
  /// \code
  /// shader.setUniform("current", meow::Shader::CurrentTexture);
  /// \endcode
  ///
  /// \param name Name of the texture in the shader
  ///
  ...
  /// mix meow::Shader with OpenGL code.
  ///
  /// \code
  /// meow::Shader s1, s2;
  /// ...
  ...
  /// any attempt to use meow::Shader will fail.
  ///
  ...
  /// any attempt to use meow::Shader geometry shader features will fail.
  ///
...
///
/// A meow::Shader can be composed of either a vertex shader
...
/// meow::Shader handles different types of uniforms:
...
/// \li meow::Color as a 4D vector (\p vec4)
/// \li meow::Transform as matrices (\p mat3 or \p mat4)
///
...
/// defined in the meow::Glsl namespace:
/// \code
/// shader.setUniform("offset", 2.f);
/// shader.setUniform("point", meow::Vector3f(0.5f, 0.8f, 0.3f));
/// shader.setUniform("color", meow::Glsl::Vec4(color));          // color is a
/// meow::Color shader.setUniform("matrix", meow::Glsl::Mat4(transform));     //
/// transform is a meow::Transform shader.setUniform("overlay", texture); //
/// texture is a meow::Texture shader.setUniform("current",
/// meow::Shader::CurrentTexture); \endcode
///
...
/// not interesting. For example, using a vertex shader on a meow::Sprite
/// is limited because there are only 4 vertices, the sprite would
/// have to be subdivided in order to apply wave effects.
/// Another bad example is a fragment shader with meow::Text: the texture
...
/// current contents of the target (like the old meow::PostFx class
/// in SFML 1). This can be done in two different ways:
/// \li draw everything to a meow::RenderTexture, then draw it to
///     the main target using the shader
/// \li draw everything directly to the main target, then use
///     meow::Texture::update(Window&) to copy its contents to a texture
...
///
/// Like meow::Texture that can be used as a raw OpenGL texture,
/// meow::Shader can also be used directly as a raw shader for
/// custom OpenGL geometry.
/// \code
/// meow::Shader::bind(&shader);
