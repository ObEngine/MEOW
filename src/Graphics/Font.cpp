...
// Headers
////////////////////////////////////////////////////////////
#include <meow/Graphics/Font.hpp>
...
namespace {
// FreeType callbacks that operate on a meow::InputStream
unsigned long read(FT_Stream rec, unsigned long offset, unsigned char *buffer,
                   unsigned long count) {
  meow::InputStream *stream =
      static_cast<meow::InputStream *>(rec->descriptor.pointer);
  if (static_cast<unsigned long>(stream->seek(offset)) == offset) {
    ...
    return count > 0 ? 0 : 1; // error code is 0 if we're reading, or nonzero if
                              // we're seeking
}
...
// Combine outline thickness, boldness and font glyph index into a single 64-bit
// key
meow::Uint64 combine(float outlineThickness, bool bold, meow::Uint32 index) {
  return (static_cast<meow::Uint64>(reinterpret<meow::Uint32>(outlineThickness))
          << 32) |
         (static_cast<meow::Uint64>(bold) << 31) | index;
}
} // namespace
...
////////////////////////////////////////////////////////////
Font::Font()
    : m_library(NULL), m_face(NULL), m_streamRec(NULL), m_stroker(NULL),
      m_refCount(NULL), m_isSmooth(true), m_info() {
#ifdef SFML_SYSTEM_ANDROID
  m_stream = NULL;
#endif
}
...
Font::Font(const Font &copy)
    : m_library(copy.m_library), m_face(copy.m_face),
      m_streamRec(copy.m_streamRec), m_stroker(copy.m_stroker),
      m_refCount(copy.m_refCount), m_isSmooth(copy.m_isSmooth),
      m_info(copy.m_info), m_pages(copy.m_pages),
...
bool Font::loadFromFile(const std::string &filename) {
  ...
  // Initialize FreeType
  // Note: we initialize FreeType for every font instance in order to avoid
  // having a single global manager that would create a lot of issues regarding
  // creation and destruction order.
  FT_Library library;
  if (FT_Init_FreeType(&library) != 0) {
    err() << "Failed to load font \"" << filename
          << "\" (failed to initialize FreeType)" << std::endl;
    return false;
  }
  m_library = library;
  ...
  FT_Face face;
  if (FT_New_Face(static_cast<FT_Library>(m_library), filename.c_str(), 0,
                  &face) != 0) {
    err() << "Failed to load font \"" << filename
          << "\" (failed to create the font face)" << std::endl;
    return false;
  }
  ...
  if (FT_Stroker_New(static_cast<FT_Library>(m_library), &stroker) != 0) {
    err() << "Failed to load font \"" << filename
          << "\" (failed to create the stroker)" << std::endl;
    FT_Done_Face(face);
    return false;
  }
...
bool Font::loadFromMemory(const void *data, std::size_t sizeInBytes) {
  ...
  FT_Library library;
  if (FT_Init_FreeType(&library) != 0) {
    err() << "Failed to load font from memory (failed to initialize FreeType)"
          << std::endl;
    return false;
  }
  m_library = library;
  ...
  if (FT_New_Memory_Face(static_cast<FT_Library>(m_library),
    ...
    err() << "Failed to load font from memory (failed to create the font face)"
          << std::endl;
    return false;
  }
  ...
  FT_Stroker stroker;
  if (FT_Stroker_New(static_cast<FT_Library>(m_library), &stroker) != 0) {
    err() << "Failed to load font from memory (failed to create the stroker)"
          << std::endl;
    FT_Done_Face(face);
    return false;
  }
  ...
  // Select the Unicode character map
  if (FT_Select_Charmap(face, FT_ENCODING_UNICODE) != 0) {
    err() << "Failed to load font from memory (failed to set the Unicode "
             "character set)"
          << std::endl;
    FT_Stroker_Done(stroker);
    FT_Done_Face(face);
...
bool Font::loadFromStream(InputStream &stream) {
  ...
  FT_Library library;
  if (FT_Init_FreeType(&library) != 0) {
    err() << "Failed to load font from stream (failed to initialize FreeType)"
          << std::endl;
    return false;
  }
  m_library = library;
  ...
  if (FT_Open_Face(static_cast<FT_Library>(m_library), &args, 0, &face) != 0) {
    err() << "Failed to load font from stream (failed to create the font face)"
          << std::endl;
    delete rec;
    return false;
  }
  ...
  FT_Stroker stroker;
  if (FT_Stroker_New(static_cast<FT_Library>(m_library), &stroker) != 0) {
    err() << "Failed to load font from stream (failed to create the stroker)"
          << std::endl;
    FT_Done_Face(face);
    delete rec;
    return false;
  ...
  // Select the Unicode character map
  if (FT_Select_Charmap(face, FT_ENCODING_UNICODE) != 0) {
    err() << "Failed to load font from stream (failed to set the Unicode "
             "character set)"
          << std::endl;
    FT_Done_Face(face);
    FT_Stroker_Done(stroker);
...
const Glyph &Font::getGlyph(Uint32 codePoint, unsigned int characterSize,
  ...
  // Build the key by combining the glyph index (based on code point), bold
  // flag, and outline thickness
  Uint64 key =
...
float Font::getKerning(Uint32 first, Uint32 second, unsigned int characterSize,
  ...
  if (face && setCurrentSize(characterSize)) {
    ...
    // Retrieve position compensation deltas generated by FT_LOAD_FORCE_AUTOHINT
    // flag
    ...
    // Flooring is required as we use FT_KERNING_UNFITTED flag which is not
    // quantized in 64 based grid
    return std::floor(
...
void Font::setSmooth(bool smooth) {
  if (smooth != m_isSmooth) {
    ...
    for (meow::Font::PageTable::iterator page = m_pages.begin();
...
void Font::cleanup() {
  ...
  if (m_refCount) {
    ...
    if (*m_refCount == 0) {
      ...
      // Destroy the stream rec instance, if any (must be done after
      // FT_Done_Face!)
      if (m_streamRec)
...
Glyph Font::loadGlyph(Uint32 codePoint, unsigned int characterSize, bool bold,
  ...
  if (!outline) {
    if (bold)
      FT_Bitmap_Embolden(static_cast<FT_Library>(m_library), &bitmap, weight,
                         weight);
  ...
  if ((width > 0) && (height > 0)) {
    ...
    glyph.bounds.left = static_cast<float>(face->glyph->metrics.horiBearingX) /
                        static_cast<float>(1 << 6);
    glyph.bounds.top = -static_cast<float>(face->glyph->metrics.horiBearingY) /
                       static_cast<float>(1 << 6);
    glyph.bounds.width = static_cast<float>(face->glyph->metrics.width) /
                             static_cast<float>(1 << 6) +
                         outlineThickness * 2;
    glyph.bounds.height = static_cast<float>(face->glyph->metrics.height) /
                              static_cast<float>(1 << 6) +
                          outlineThickness * 2;

    // Resize the pixel buffer to the new size and fill it with transparent
    // white pixels
      ...
      for (unsigned int y = padding; y < height - padding; ++y) {
        for (unsigned int x = padding; x < width - padding; ++x) {
          ...
          m_pixelBuffer[index * 4 + 3] =
              ((pixels[(x - padding) / 8]) & (1 << (7 - ((x - padding) % 8))))
                  ? 255
                  : 0;
        }
        pixels += bitmap.pitch;
...
////////////////////////////////////////////////////////////
IntRect Font::findGlyphRect(Page &page, unsigned int width,
                            unsigned int height) const {
  // Find the line that fits well the glyph
  Row *row = NULL;
  float bestRatio = 0;
  for (std::vector<Row>::iterator it = page.rows.begin();
       it != page.rows.end() && !row; ++it) {
  ...
  // If we didn't find a matching row, create a new one (10% taller than the
  // glyph)
  if (!row) {
    ...
    while ((page.nextRow + rowHeight >= page.texture.getSize().y) ||
      ...
      if ((textureWidth * 2 <= Texture::getMaximumSize()) &&
        ...
        err() << "Failed to add a new character to the font: the maximum "
                 "texture size has been reached"
              << std::endl;
