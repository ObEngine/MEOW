...
bool Image::loadFromFile(const std::string &filename) {
  ...
  return priv::ImageLoader::getInstance().loadImageFromFile(filename, m_pixels,
                                                            m_size);
...
bool Image::loadFromMemory(const void *data, std::size_t size) {
  return priv::ImageLoader::getInstance().loadImageFromMemory(data, size,
                                                              m_pixels, m_size);
}

////////////////////////////////////////////////////////////
bool Image::loadFromStream(InputStream &stream) {
  return priv::ImageLoader::getInstance().loadImageFromStream(stream, m_pixels,
                                                              m_size);
}

////////////////////////////////////////////////////////////
bool Image::saveToFile(const std::string &filename) const {
  return priv::ImageLoader::getInstance().saveImageToFile(filename, m_pixels,
                                                          m_size);
}

////////////////////////////////////////////////////////////
bool Image::saveToMemory(std::vector<meow::Uint8> &output,
                         const std::string &format) const {
  return priv::ImageLoader::getInstance().saveImageToMemory(format, output,
                                                            m_pixels, m_size);
}
...
////////////////////////////////////////////////////////////
void Image::copy(const Image &source, unsigned int destX, unsigned int destY,
                 const IntRect &sourceRect, bool applyAlpha) {
  // Make sure that both images are valid
  if ((source.m_size.x == 0) || (source.m_size.y == 0) || (m_size.x == 0) ||
      (m_size.y == 0))
    return;
  ...
  const Uint8 *srcPixels =
      &source.m_pixels[0] + (srcRect.left + srcRect.top * source.m_size.x) * 4;
...
void Image::flipHorizontally() {
  if (!m_pixels.empty()) {
    ...
    for (std::size_t y = 0; y < m_size.y; ++y) {
      ...
      std::vector<Uint8>::iterator right =
          m_pixels.begin() + (y + 1) * rowSize - 4;
