...
// Headers
////////////////////////////////////////////////////////////
#include <meow/Graphics/ImageLoader.hpp>
...
// stb_image callbacks that operate on a meow::InputStream
int read(void *user, char *data, int size) {
  meow::InputStream *stream = static_cast<meow::InputStream *>(user);
...
void bufferFromCallback(void *context, void *data, int size) {
  meow::Uint8 *source = static_cast<meow::Uint8 *>(data);
  std::vector<meow::Uint8> *dest =
      static_cast<std::vector<meow::Uint8> *>(context);
...
bool ImageLoader::loadImageFromFile(const std::string &filename,
  ...
  if (ptr) {
    ...
    err() << "Failed to load image \"" << filename
...
////////////////////////////////////////////////////////////
bool ImageLoader::loadImageFromMemory(const void *data, std::size_t dataSize,
                                      std::vector<Uint8> &pixels,
                                      Vector2u &size) {
...
////////////////////////////////////////////////////////////
bool ImageLoader::loadImageFromStream(InputStream &stream,
                                      std::vector<Uint8> &pixels,
                                      Vector2u &size) {
  // Clear the array (just in case)
  pixels.clear();
  ...
  int height = 0;
  int channels = 0;
  unsigned char *ptr = stbi_load_from_callbacks(
  ...
  if (ptr) {
    ...
    // Error, failed to load the image
    err() << "Failed to load image from stream. Reason: "
          << stbi_failure_reason() << std::endl;
...
bool ImageLoader::saveImageToFile(const std::string &filename,
  ...
  if (!pixels.empty() && (size.x > 0) && (size.y > 0)) {
    ...
    const std::string extension =
        dot != std::string::npos ? toLower(filename.substr(dot + 1)) : "";
...
bool ImageLoader::saveImageToMemory(const std::string &format,
  ...
  if (!pixels.empty() && (size.x > 0) && (size.y > 0)) {
    ...
    if (specified == "bmp") {
      // BMP format
      if (stbi_write_bmp_to_func(&bufferFromCallback, &output, size.x, size.y,
                                 4, &pixels[0]))
        return true;
    } else if (specified == "tga") {
      // TGA format
      if (stbi_write_tga_to_func(&bufferFromCallback, &output, size.x, size.y,
                                 4, &pixels[0]))
        return true;
    } else if (specified == "png") {
      // PNG format
      if (stbi_write_png_to_func(&bufferFromCallback, &output, size.x, size.y,
                                 4, &pixels[0], 0))
        return true;
    } else if (specified == "jpg" || specified == "jpeg") {
      // JPG format
      if (stbi_write_jpg_to_func(&bufferFromCallback, &output, size.x, size.y,
                                 4, &pixels[0], 90))
        return true;
    }
  }
