...
#include <iostream>
#include <map>
#include <meow/Graphics/Drawable.hpp>
...
meow::Uint64 getUniqueId() {
  meow::Lock lock(mutex);

  static meow::Uint64 id = 1; // start at 1, zero is "no RenderTarget"
...
typedef std::map<meow::Uint64, meow::Uint64> ContextRenderTargetMap;
ContextRenderTargetMap contextRenderTargetMap;

// Check if a RenderTarget with the given ID is active in the current context
bool isActive(meow::Uint64 id) {
  ContextRenderTargetMap::iterator iter =
      contextRenderTargetMap.find(meow::Context::getActiveContextId());
...
// Convert an meow::BlendMode::Factor constant to the corresponding OpenGL
// constant.
meow::Uint32 factorToGlConstant(meow::BlendMode::Factor blendFactor) {
  switch (blendFactor) {
  case meow::BlendMode::Zero:
    return GL_ZERO;
  case meow::BlendMode::One:
    return GL_ONE;
  case meow::BlendMode::SrcColor:
    return GL_SRC_COLOR;
  case meow::BlendMode::OneMinusSrcColor:
    return GL_ONE_MINUS_SRC_COLOR;
  case meow::BlendMode::DstColor:
    return GL_DST_COLOR;
  case meow::BlendMode::OneMinusDstColor:
    return GL_ONE_MINUS_DST_COLOR;
  case meow::BlendMode::SrcAlpha:
    return GL_SRC_ALPHA;
  case meow::BlendMode::OneMinusSrcAlpha:
    return GL_ONE_MINUS_SRC_ALPHA;
  case meow::BlendMode::DstAlpha:
    return GL_DST_ALPHA;
  case meow::BlendMode::OneMinusDstAlpha:
    return GL_ONE_MINUS_DST_ALPHA;
  }

  meow::err() << "Invalid value for meow::BlendMode::Factor! Fallback to "
                 "meow::BlendMode::Zero."
              << std::endl;
  assert(false);
  return GL_ZERO;
...
// Convert an meow::BlendMode::BlendEquation constant to the corresponding
// OpenGL constant.
meow::Uint32 equationToGlConstant(meow::BlendMode::Equation blendEquation) {
  switch (blendEquation) {
  case meow::BlendMode::Add:
    return GLEXT_GL_FUNC_ADD;
  case meow::BlendMode::Subtract:
    if (GLEXT_blend_subtract)
      return GLEXT_GL_FUNC_SUBTRACT;
    break;
  case meow::BlendMode::ReverseSubtract:
    if (GLEXT_blend_subtract)
      return GLEXT_GL_FUNC_REVERSE_SUBTRACT;
    break;
  case meow::BlendMode::Min:
    if (GLEXT_blend_minmax)
      return GLEXT_GL_MIN;
    break;
  case meow::BlendMode::Max:
    if (GLEXT_blend_minmax)
      return GLEXT_GL_MAX;
    break;
  }
  ...
  if (!warned) {
    meow::err()
        ...
        << std::endl;
    meow::err()
        << "Some blending equations will fallback to meow::BlendMode::Add"
        << std::endl;
    meow::err() << "Ensure that hardware acceleration is enabled if available"
                << std::endl;
...
void RenderTarget::clear(const Color &color) {
  if (RenderTargetImpl::isActive(m_id) || setActive(true)) {
    ...
    glCheck(glClearColor(color.r / 255.f, color.g / 255.f, color.b / 255.f,
                         color.a / 255.f));
...
////////////////////////////////////////////////////////////
const View &RenderTarget::getView() const { return m_view; }

////////////////////////////////////////////////////////////
const View &RenderTarget::getDefaultView() const { return m_defaultView; }
...
Vector2i RenderTarget::mapCoordsToPixel(const Vector2f &point,
  ...
  Vector2i pixel;
  IntRect viewport = getViewport(view);
  pixel.x = static_cast<int>((normalized.x + 1.f) / 2.f * viewport.width +
                             viewport.left);
  pixel.y = static_cast<int>((-normalized.y + 1.f) / 2.f * viewport.height +
                             viewport.top);
...
// GL_QUADS is unavailable on OpenGL ES
  ...
  if (type == Quads) {
    err() << "meow::Quads primitive type is not supported on OpenGL ES "
             "platforms, drawing skipped"
          << std::endl;
    return;
  }
#endif
  ...
  if (RenderTargetImpl::isActive(m_id) || setActive(true)) {
    ...
    // Check if texture coordinates array is needed, and update client state
    // accordingly
...
void RenderTarget::draw(const VertexBuffer &vertexBuffer,
  ...
  if (!VertexBuffer::isAvailable()) {
    err() << "meow::VertexBuffer is not available, drawing skipped"
          << std::endl;
    return;
  }
  ...
  vertexCount =
      std::min(vertexCount, vertexBuffer.getVertexCount() - firstVertex);
...
bool RenderTarget::setActive(bool active) {
  ...
  {
    meow::Lock lock(RenderTargetImpl::mutex);

    Uint64 contextId = Context::getActiveContextId();

    using RenderTargetImpl::contextRenderTargetMap;
    RenderTargetImpl::ContextRenderTargetMap::iterator iter =
        contextRenderTargetMap.find(contextId);
...
void RenderTarget::resetGLStates() {
  // Check here to make sure a context change does not happen after
  // activate(true)
...
void RenderTarget::initialize() {
  ...
  m_defaultView.reset(FloatRect(0, 0, static_cast<float>(getSize().x),
...
void RenderTarget::applyBlendMode(const BlendMode &mode) {
  ...
  // Apply the blend mode, falling back to the non-separate versions if
  // necessary
  ...
  if (GLEXT_blend_minmax || GLEXT_blend_subtract) {
    ...
    if (!warned) {
#ifdef SFML_OPENGL_ES
      err() << "OpenGL ES extension OES_blend_subtract unavailable"
            << std::endl;
#else
      err() << "OpenGL extension EXT_blend_minmax and EXT_blend_subtract "
               "unavailable"
            << std::endl;
#endif
      err() << "Selecting a blend equation not possible" << std::endl;
      err() << "Ensure that hardware acceleration is enabled if available"
            << std::endl;
...
////////////////////////////////////////////////////////////
void RenderTarget::applyShader(const Shader *shader) { Shader::bind(shader); }

////////////////////////////////////////////////////////////
void RenderTarget::setupDraw(bool useVertexCache, const RenderStates &states) {
  // Enable or disable sRGB encoding
  // This is needed for drivers that do not check the format of the surface
  // drawn to before applying sRGB conversion
  if (!m_cache.enable) {
    if (isSrgb())
  ...
  if (useVertexCache) {
    // Since vertices are transformed, we must use an identity transform to
    // render them
...
////////////////////////////////////////////////////////////
void RenderTarget::drawPrimitives(PrimitiveType type, std::size_t firstVertex,
...
void RenderTarget::cleanupDraw(const RenderStates &states) {
  ...
  // If the texture we used to draw belonged to a RenderTexture, then forcibly
  // unbind that texture. This prevents a bug where some drivers do not clear
...
//   is not enough; if the meow::Texture instance is destroyed,
