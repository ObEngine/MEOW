...
std::set<std::map<meow::Uint64, unsigned int> *> frameBuffers;
...
std::set<std::pair<meow::Uint64, unsigned int>> staleFrameBuffers;
...
void destroyStaleFBOs() {
  meow::Uint64 contextId = meow::Context::getActiveContextId();

  for (std::set<std::pair<meow::Uint64, unsigned int>>::iterator iter =
           staleFrameBuffers.begin();
       iter != staleFrameBuffers.end();) {
...
void contextDestroyCallback(void *arg) {
  meow::Lock lock(mutex);

  meow::Uint64 contextId = meow::Context::getActiveContextId();

  // Destroy active frame buffer objects
  for (std::set<std::map<meow::Uint64, unsigned int> *>::iterator
           frameBuffersIter = frameBuffers.begin();
       frameBuffersIter != frameBuffers.end(); ++frameBuffersIter) {
    for (std::map<meow::Uint64, unsigned int>::iterator iter =
             (*frameBuffersIter)->begin();
         iter != (*frameBuffersIter)->end(); ++iter) {
      if (iter->first == contextId) {
        GLuint frameBuffer = static_cast<GLuint>(iter->second);
        glCheck(GLEXT_glDeleteFramebuffers(1, &frameBuffer));
...
RenderTextureImplFBO::RenderTextureImplFBO()
    : m_depthStencilBuffer(0), m_colorBuffer(0), m_width(0), m_height(0),
      m_context(NULL), m_textureId(0), m_multisample(false), m_stencil(false),
      m_sRgb(false) {
  Lock lock(mutex);
...
RenderTextureImplFBO::~RenderTextureImplFBO() {
  ...
  for (std::map<Uint64, unsigned int>::iterator iter = m_frameBuffers.begin();
       iter != m_frameBuffers.end(); ++iter)
    staleFrameBuffers.insert(std::make_pair(iter->first, iter->second));

  for (std::map<Uint64, unsigned int>::iterator iter =
           m_multisampleFrameBuffers.begin();
       iter != m_multisampleFrameBuffers.end(); ++iter)
    staleFrameBuffers.insert(std::make_pair(iter->first, iter->second));
...
////////////////////////////////////////////////////////////
bool RenderTextureImplFBO::create(unsigned int width, unsigned int height,
...

  {
    ...
    if (settings.antialiasingLevel) {
      ...
      if (settings.antialiasingLevel > static_cast<unsigned int>(samples)) {
        err() << "Impossible to create render texture (unsupported "
                 "anti-aliasing level)";
        err() << " Requested: " << settings.antialiasingLevel
              << " Maximum supported: " << samples << std::endl;
        return false;
      }
    }
    ...
    if (!settings.antialiasingLevel) {
      ...
      if (settings.stencilBits) {
        ...
        if (!m_depthStencilBuffer) {
          err() << "Impossible to create render texture (failed to create the "
                   "attached depth/stencil buffer)"
                << std::endl;
          return false;
        }
        glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER,
                                         m_depthStencilBuffer));
        glCheck(GLEXT_glRenderbufferStorage(
            GLEXT_GL_RENDERBUFFER, GLEXT_GL_DEPTH24_STENCIL8, width, height));
        ...
        glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER,
                                         m_depthStencilBuffer));
        glCheck(GLEXT_glRenderbufferStorage(
            GLEXT_GL_RENDERBUFFER, GLEXT_GL_DEPTH_COMPONENT, width, height));
      }
    } else {
      ...
      if (!m_colorBuffer) {
        err() << "Impossible to create render texture (failed to create the "
                 "attached multisample color buffer)"
              << std::endl;
        return false;
      }
      glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER, m_colorBuffer));
      glCheck(GLEXT_glRenderbufferStorageMultisample(
          GLEXT_GL_RENDERBUFFER, settings.antialiasingLevel,
          m_sRgb ? GL_SRGB8_ALPHA8_EXT : GL_RGBA, width, height));
      ...
      if (settings.stencilBits) {
        ...
        if (!m_depthStencilBuffer) {
          err() << "Impossible to create render texture (failed to create the "
                   "attached multisample depth/stencil buffer)"
                << std::endl;
          return false;
        }
        glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER,
        ...
        glCheck(GLEXT_glRenderbufferStorageMultisample(
            GLEXT_GL_RENDERBUFFER, settings.antialiasingLevel,
            GLEXT_GL_DEPTH24_STENCIL8, width, height));
        ...
        if (!m_depthStencilBuffer) {
          err() << "Impossible to create render texture (failed to create the "
                   "attached multisample depth buffer)"
                << std::endl;
          return false;
        }
        glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER,
        ...
        glCheck(GLEXT_glRenderbufferStorageMultisample(
            GLEXT_GL_RENDERBUFFER, settings.antialiasingLevel,
            GLEXT_GL_DEPTH_COMPONENT, width, height));
      }

#else

      err() << "Impossible to create render texture (failed to create the "
               "multisample render buffers)"
            << std::endl;
      return false;
...
bool RenderTextureImplFBO::createFrameBuffer() {
  ...
  if (!frameBuffer) {
    err() << "Impossible to create render texture (failed to create the frame "
             "buffer object)"
          << std::endl;
    return false;
  }
  ...
  if (!m_multisample && m_depthStencilBuffer) {
    glCheck(GLEXT_glFramebufferRenderbuffer(
        GLEXT_GL_FRAMEBUFFER, GLEXT_GL_DEPTH_ATTACHMENT, GLEXT_GL_RENDERBUFFER,
        m_depthStencilBuffer));
    ...
    if (m_stencil) {
      glCheck(GLEXT_glFramebufferRenderbuffer(
          GLEXT_GL_FRAMEBUFFER, GLEXT_GL_STENCIL_ATTACHMENT,
          GLEXT_GL_RENDERBUFFER, m_depthStencilBuffer));
    }
  ...
  if (status != GLEXT_GL_FRAMEBUFFER_COMPLETE) {
    glCheck(GLEXT_glBindFramebuffer(GLEXT_GL_FRAMEBUFFER, 0));
    glCheck(GLEXT_glDeleteFramebuffers(1, &frameBuffer));
    err() << "Impossible to create render texture (failed to link the target "
...

  {
    ...
    m_frameBuffers.insert(std::make_pair(
        Context::getActiveContextId(), static_cast<unsigned int>(frameBuffer)));
  }
  ...
  if (m_multisample) {
    ...
    if (!multisampleFrameBuffer) {
      err() << "Impossible to create render texture (failed to create the "
               "multisample frame buffer object)"
            << std::endl;
      return false;
    }
    glCheck(
        GLEXT_glBindFramebuffer(GLEXT_GL_FRAMEBUFFER, multisampleFrameBuffer));

    // Link the multisample color buffer to the frame buffer
    glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER, m_colorBuffer));
    glCheck(GLEXT_glFramebufferRenderbuffer(
        GLEXT_GL_FRAMEBUFFER, GLEXT_GL_COLOR_ATTACHMENT0, GLEXT_GL_RENDERBUFFER,
        m_colorBuffer));

    // Link the depth/stencil renderbuffer to the frame buffer
    if (m_depthStencilBuffer) {
      glCheck(GLEXT_glFramebufferRenderbuffer(
          GLEXT_GL_FRAMEBUFFER, GLEXT_GL_DEPTH_ATTACHMENT,
          GLEXT_GL_RENDERBUFFER, m_depthStencilBuffer));
    ...
    if (status != GLEXT_GL_FRAMEBUFFER_COMPLETE) {
      ...
      err() << "Impossible to create render texture (failed to link the render "
               "buffers to the multisample frame buffer)"
            << std::endl;
      return false;
    }

    {
      ...
      // Insert the FBO into our map
      m_multisampleFrameBuffers.insert(
          std::make_pair(Context::getActiveContextId(),
                         static_cast<unsigned int>(multisampleFrameBuffer)));
    }
  }
...
bool RenderTextureImplFBO::activate(bool active) {
  ...
  if (!contextId) {
    ...
    if (!contextId) {
      err() << "Impossible to activate render texture (failed to create backup "
               "context)"
            << std::endl;
...
////////////////////////////////////////////////////////////
bool RenderTextureImplFBO::isSrgb() const { return m_sRgb; }
...
void RenderTextureImplFBO::updateTexture(unsigned int) {
  ...
  if (m_multisample && m_width && m_height && activate(true)) {
    ...
    std::map<Uint64, unsigned int>::iterator iter =
        m_frameBuffers.find(contextId);
    std::map<Uint64, unsigned int>::iterator multisampleIter =
        m_multisampleFrameBuffers.find(contextId);

    if ((iter != m_frameBuffers.end()) &&
        (multisampleIter != m_multisampleFrameBuffers.end())) {
      // Set up the blit target (draw framebuffer) and blit (from the read
      // framebuffer, our multisample FBO)
      glCheck(GLEXT_glBindFramebuffer(GLEXT_GL_DRAW_FRAMEBUFFER, iter->second));
      glCheck(GLEXT_glBlitFramebuffer(0, 0, m_width, m_height, 0, 0, m_width,
                                      m_height, GL_COLOR_BUFFER_BIT,
                                      GL_NEAREST));
      glCheck(GLEXT_glBindFramebuffer(GLEXT_GL_DRAW_FRAMEBUFFER,
                                      multisampleIter->second));
    }
  }
