...
#include <meow/Graphics/RenderWindow.hpp>
...
RenderWindow::RenderWindow(VideoMode mode, const String &title, Uint32 style,
  ...
  // Don't call the base class constructor because it contains virtual function
  // calls
...
////////////////////////////////////////////////////////////
bool RenderWindow::isSrgb() const { return getSettings().sRgbCapable; }
...
void RenderWindow::onCreate() {
  if (priv::RenderTextureImplFBO::isAvailable()) {
    // Retrieve the framebuffer ID we have to bind when targeting the window for
    // rendering We assume that this window's context is still active at this
    // point
    glCheck(glGetIntegerv(GLEXT_GL_FRAMEBUFFER_BINDING,
...
void RenderWindow::onResize() {
  // Update the current view (recompute the viewport, which is stored in
