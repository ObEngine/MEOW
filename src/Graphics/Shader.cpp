...
#include <meow/Graphics/Shader.hpp>
#include <meow/Graphics/Texture.hpp>
#include <meow/Graphics/Transform.hpp>
#include <meow/System/Err.hpp>
#include <meow/System/InputStream.hpp>
...
#define castToGlHandle(x)                                                      \
  reinterpret_cast<GLEXT_GLhandle>(static_cast<ptrdiff_t>(x))
#define castFromGlHandle(x)                                                    \
  static_cast<unsigned int>(reinterpret_cast<ptrdiff_t>(x))
...
namespace {
meow::Mutex maxTextureUnitsMutex;
meow::Mutex isAvailableMutex;
...
GLint getMaxTextureUnits() {
  ...
  meow::Lock lock(maxTextureUnitsMutex);
...
bool getStreamContents(meow::InputStream &stream, std::vector<char> &buffer) {
  bool success = true;
  meow::Int64 size = stream.getSize();
  if (size > 0) {
    buffer.resize(static_cast<std::size_t>(size));
    stream.seek(0);
    meow::Int64 read = stream.read(&buffer[0], size);
    success = (read == size);
  }
  buffer.push_back('\0');
  return success;
...
template <typename T>
std::vector<T> flatten(const meow::Vector2<T> *vectorArray,
                       std::size_t length) {
...
template <typename T>
std::vector<T> flatten(const meow::Vector3<T> *vectorArray,
                       std::size_t length) {
...
template <typename T>
std::vector<T> flatten(const meow::priv::Vector4<T> *vectorArray,
                       std::size_t length) {
...
struct Shader::UniformBinder : private NonCopyable {
  ...
  UniformBinder(Shader &shader, const std::string &name)
      : savedProgram(0), currentProgram(castToGlHandle(shader.m_shaderProgram)),
        location(-1) {
    if (currentProgram) {
  ...
  TransientContextLock
      lock; //!< Lock to keep context active while uniform is bound
  GLEXT_GLhandle
      savedProgram; //!< Handle to the previously active program object
  GLEXT_GLhandle currentProgram; //!< Handle to the program object of the
                                 //!< modified meow::Shader instance
  GLint
      location; //!< Uniform location, used by the surrounding meow::Shader code
};

////////////////////////////////////////////////////////////
Shader::Shader()
    : m_shaderProgram(0), m_currentTexture(-1), m_textures(), m_uniforms() {}
...
bool Shader::loadFromFile(const std::string &vertexShaderFilename,
  ...
  if (!getFileContents(vertexShaderFilename, vertexShader)) {
    err() << "Failed to open vertex shader file \"" << vertexShaderFilename
          << "\"" << std::endl;
    return false;
  }
  ...
  if (!getFileContents(fragmentShaderFilename, fragmentShader)) {
    err() << "Failed to open fragment shader file \"" << fragmentShaderFilename
          << "\"" << std::endl;
    return false;
  }
...
bool Shader::loadFromFile(const std::string &vertexShaderFilename,
  ...
  if (!getFileContents(geometryShaderFilename, geometryShader)) {
    err() << "Failed to open geometry shader file \"" << geometryShaderFilename
          << "\"" << std::endl;
    return false;
  }
...
bool Shader::loadFromMemory(const std::string &vertexShader,
                            ...
                            const std::string &fragmentShader) {
  // Compile the shader program
  return compile(vertexShader.c_str(), geometryShader.c_str(),
...
void Shader::setUniform(const std::string &name, const Glsl::Mat3 &matrix) {
  ...
  if (binder.location != -1)
    glCheck(
        GLEXT_glUniformMatrix3fv(binder.location, 1, GL_FALSE, matrix.array));
}
...
void Shader::setUniform(const std::string &name, const Glsl::Mat4 &matrix) {
  ...
  if (binder.location != -1)
    glCheck(
        GLEXT_glUniformMatrix4fv(binder.location, 1, GL_FALSE, matrix.array));
}
...
////////////////////////////////////////////////////////////
void Shader::setUniformArray(const std::string &name, const float *scalarArray,
                             std::size_t length) {
  UniformBinder binder(*this, name);
  if (binder.location != -1)
    glCheck(GLEXT_glUniform1fv(binder.location, static_cast<GLsizei>(length),
                               scalarArray));
}
...
void Shader::setUniformArray(const std::string &name,
  ...
  UniformBinder binder(*this, name);
  if (binder.location != -1)
    glCheck(GLEXT_glUniform2fv(binder.location, static_cast<GLsizei>(length),
                               &contiguous[0]));
}
...
void Shader::setUniformArray(const std::string &name,
  ...
  UniformBinder binder(*this, name);
  if (binder.location != -1)
    glCheck(GLEXT_glUniform3fv(binder.location, static_cast<GLsizei>(length),
                               &contiguous[0]));
}
...
void Shader::setUniformArray(const std::string &name,
  ...
  UniformBinder binder(*this, name);
  if (binder.location != -1)
    glCheck(GLEXT_glUniform4fv(binder.location, static_cast<GLsizei>(length),
                               &contiguous[0]));
}
...
void Shader::setUniformArray(const std::string &name,
  ...
  for (std::size_t i = 0; i < length; ++i)
    priv::copyMatrix(matrixArray[i].array, matrixSize,
                     &contiguous[matrixSize * i]);

  UniformBinder binder(*this, name);
  if (binder.location != -1)
    glCheck(GLEXT_glUniformMatrix3fv(binder.location,
                                     static_cast<GLsizei>(length), GL_FALSE,
                                     &contiguous[0]));
}
...
void Shader::setUniformArray(const std::string &name,
  ...
  UniformBinder binder(*this, name);
  if (binder.location != -1)
    glCheck(GLEXT_glUniformMatrix4fv(binder.location,
                                     static_cast<GLsizei>(length), GL_FALSE,
                                     &contiguous[0]));
}
...
////////////////////////////////////////////////////////////
unsigned int Shader::getNativeHandle() const { return m_shaderProgram; }
...
void Shader::bind(const Shader *shader) {
  ...
  if (!isAvailable()) {
    err() << "Failed to bind or unbind shader: your system doesn't support "
             "shaders "
          << "(you should test Shader::isAvailable() before trying to use the "
             "Shader class)"
          << std::endl;
    return;
  }
...
bool Shader::isAvailable() {
  ...
  if (!checked) {
    ...
    meow::priv::ensureExtensionsInit();

    available = GLEXT_multitexture && GLEXT_shading_language_100 &&
                GLEXT_shader_objects && GLEXT_vertex_shader &&
...
bool Shader::isGeometryAvailable() {
  ...
  if (!checked) {
    ...
    available =
        isAvailable() && (GLEXT_geometry_shader4 || GLEXT_GL_VERSION_3_2);
  }
...
////////////////////////////////////////////////////////////
bool Shader::compile(const char *vertexShaderCode,
  ...
  if (geometryShaderCode && !isGeometryAvailable()) {
    err() << "Failed to create a shader: your system doesn't support geometry "
             "shaders "
          << "(you should test Shader::isGeometryAvailable() before trying to "
  ...
  if (vertexShaderCode) {
    ...
    GLint success;
    glCheck(GLEXT_glGetObjectParameteriv(
        vertexShader, GLEXT_GL_OBJECT_COMPILE_STATUS, &success));
    if (success == GL_FALSE) {
      char log[1024];
  ...
  if (geometryShaderCode) {
    // Create and compile the shader
    GLEXT_GLhandle geometryShader =
        GLEXT_glCreateShaderObject(GLEXT_GL_GEOMETRY_SHADER);
    ...
    GLint success;
    glCheck(GLEXT_glGetObjectParameteriv(
        geometryShader, GLEXT_GL_OBJECT_COMPILE_STATUS, &success));
    if (success == GL_FALSE) {
      char log[1024];
  ...
  if (fragmentShaderCode) {
    ...
    GLint success;
    glCheck(GLEXT_glGetObjectParameteriv(
        fragmentShader, GLEXT_GL_OBJECT_COMPILE_STATUS, &success));
    if (success == GL_FALSE) {
      char log[1024];
  ...
  // Check the link log
  GLint success;
  glCheck(GLEXT_glGetObjectParameteriv(shaderProgram,
                                       GLEXT_GL_OBJECT_LINK_STATUS, &success));
  if (success == GL_FALSE) {
    char log[1024];
    glCheck(GLEXT_glGetInfoLog(shaderProgram, sizeof(log), 0, log));
    err() << "Failed to link shader:" << std::endl << log << std::endl;
    glCheck(GLEXT_glDeleteObject(shaderProgram));
    return false;
  }
...
int Shader::getUniformLocation(const std::string &name) {
  ...
  if (it != m_uniforms.end()) {
    ...
    int location = GLEXT_glGetUniformLocation(castToGlHandle(m_shaderProgram),
                                              name.c_str());
...
// OpenGL ES 1 doesn't support GLSL shaders at all, we have to provide an empty
// implementation
...
////////////////////////////////////////////////////////////
bool Shader::loadFromStream(InputStream &stream, Type type) { return false; }
...
////////////////////////////////////////////////////////////
unsigned int Shader::getNativeHandle() const { return 0; }

////////////////////////////////////////////////////////////
void Shader::bind(const Shader *shader) {}

////////////////////////////////////////////////////////////
bool Shader::isAvailable() { return false; }

////////////////////////////////////////////////////////////
bool Shader::isGeometryAvailable() { return false; }
