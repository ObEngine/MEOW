...
#include <meow/Graphics/Text.hpp>
#include <meow/Graphics/Texture.hpp>

namespace {
// Add an underline or strikethrough line to the vertex array
void addLine(meow::VertexArray &vertices, float lineLength, float lineTop,
             const meow::Color &color, float offset, float thickness,
             float outlineThickness = 0) {
  float top = std::floor(lineTop + offset - (thickness / 2) + 0.5f);
  float bottom = top + std::floor(thickness + 0.5f);

  vertices.append(
      meow::Vertex(meow::Vector2f(-outlineThickness, top - outlineThickness),
                   color, meow::Vector2f(1, 1)));
  vertices.append(meow::Vertex(
      meow::Vector2f(lineLength + outlineThickness, top - outlineThickness),
      color, meow::Vector2f(1, 1)));
  vertices.append(
      meow::Vertex(meow::Vector2f(-outlineThickness, bottom + outlineThickness),
                   color, meow::Vector2f(1, 1)));
  vertices.append(
      meow::Vertex(meow::Vector2f(-outlineThickness, bottom + outlineThickness),
                   color, meow::Vector2f(1, 1)));
  vertices.append(meow::Vertex(
      ...
      color, meow::Vector2f(1, 1)));
  vertices.append(meow::Vertex(
      meow::Vector2f(lineLength + outlineThickness, bottom + outlineThickness),
      color, meow::Vector2f(1, 1)));
}

// Add a glyph quad to the vertex array
void addGlyphQuad(meow::VertexArray &vertices, meow::Vector2f position,
                  const meow::Color &color, const meow::Glyph &glyph,
  ...
  vertices.append(meow::Vertex(
      meow::Vector2f(position.x + left - italicShear * top - outlineThickness,
                     position.y + top - outlineThickness),
      color, meow::Vector2f(u1, v1)));
  vertices.append(meow::Vertex(
      meow::Vector2f(position.x + right - italicShear * top - outlineThickness,
                     position.y + top - outlineThickness),
      color, meow::Vector2f(u2, v1)));
  vertices.append(
      meow::Vertex(meow::Vector2f(position.x + left - italicShear * bottom -
                                      outlineThickness,
                                  position.y + bottom - outlineThickness),
                   color, meow::Vector2f(u1, v2)));
  vertices.append(
      meow::Vertex(meow::Vector2f(position.x + left - italicShear * bottom -
                                      outlineThickness,
                                  position.y + bottom - outlineThickness),
  ...
  vertices.append(meow::Vertex(
      ...
      color, meow::Vector2f(u2, v1)));
  vertices.append(
      meow::Vertex(meow::Vector2f(position.x + right - italicShear * bottom -
                                      outlineThickness,
                                  position.y + bottom - outlineThickness),
                   color, meow::Vector2f(u2, v2)));
}
} // namespace
...
////////////////////////////////////////////////////////////
void Text::setColor(const Color &color) { setFillColor(color); }
...
////////////////////////////////////////////////////////////
const String &Text::getString() const { return m_string; }

////////////////////////////////////////////////////////////
const Font *Text::getFont() const { return m_font; }

////////////////////////////////////////////////////////////
unsigned int Text::getCharacterSize() const { return m_characterSize; }

////////////////////////////////////////////////////////////
float Text::getLetterSpacing() const { return m_letterSpacingFactor; }

////////////////////////////////////////////////////////////
float Text::getLineSpacing() const { return m_lineSpacingFactor; }

////////////////////////////////////////////////////////////
Uint32 Text::getStyle() const { return m_style; }

////////////////////////////////////////////////////////////
const Color &Text::getColor() const { return getFillColor(); }

////////////////////////////////////////////////////////////
const Color &Text::getFillColor() const { return m_fillColor; }

////////////////////////////////////////////////////////////
const Color &Text::getOutlineColor() const { return m_outlineColor; }

////////////////////////////////////////////////////////////
float Text::getOutlineThickness() const { return m_outlineThickness; }
...
Vector2f Text::findCharacterPos(std::size_t index) const {
  ...
  for (std::size_t i = 0; i < index; ++i) {
    ...
    position.x +=
        m_font->getKerning(prevChar, curChar, m_characterSize, isBold);
    prevChar = curChar;
...
void Text::ensureGeometryUpdate() const {
  ...
  // changed
  if (!m_geometryNeedUpdate &&
      m_font->getTexture(m_characterSize).m_cacheId == m_fontTextureId)
    return;
