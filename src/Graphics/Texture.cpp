...
#include <meow/Graphics/Image.hpp>
...
#include <meow/Window/Window.hpp>

namespace {
// A nested named namespace is used here to allow unity builds of SFML.
namespace TextureImpl {
meow::Mutex idMutex;
meow::Mutex maximumSizeMutex;

// Thread-safe unique identifier generator,
// is used for states cache (see RenderTarget)
meow::Uint64 getUniqueId() {
  meow::Lock lock(idMutex);

  static meow::Uint64 id = 1; // start at 1, zero is "no texture"
...
Texture::Texture()
    : m_size(0, 0), m_actualSize(0, 0), m_texture(0), m_isSmooth(false),
      ...
      m_fboAttachment(false), m_hasMipmap(false),
      m_cacheId(TextureImpl::getUniqueId()) {}

////////////////////////////////////////////////////////////
Texture::Texture(const Texture &copy)
    : m_size(0, 0), m_actualSize(0, 0), m_texture(0),
      m_isSmooth(copy.m_isSmooth), m_sRgb(copy.m_sRgb),
      m_isRepeated(copy.m_isRepeated), m_pixelsFlipped(false),
      m_fboAttachment(false), m_hasMipmap(false),
      m_cacheId(TextureImpl::getUniqueId()) {
  if (copy.m_texture) {
    if (create(copy.getSize().x, copy.getSize().y)) {
      ...
      err() << "Failed to copy texture, failed to create new texture"
            << std::endl;
    }
  }
}
...
bool Texture::create(unsigned int width, unsigned int height) {
  ...
  if ((width == 0) || (height == 0)) {
    err() << "Failed to create texture, invalid size (" << width << "x"
          << height << ")" << std::endl;
    return false;
  }
  ...
  static bool textureEdgeClamp =
      GLEXT_texture_edge_clamp || GLEXT_GL_VERSION_1_2 ||
  ...
  glCheck(glTexImage2D(
      GL_TEXTURE_2D, 0, (m_sRgb ? GLEXT_GL_SRGB8_ALPHA8 : GL_RGBA),
      m_actualSize.x, m_actualSize.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL));
...
bool Texture::loadFromImage(const Image &image, const IntRect &area) {
  ...
  // Load the entire image if the source area is either empty or contains the
  // whole image
  if (area.width == 0 || (area.height == 0) ||
    ...
    if (create(rectangle.width, rectangle.height)) {
      ...
      const Uint8 *pixels =
          image.getPixelsPtr() + 4 * (rectangle.left + (width * rectangle.top));
      glCheck(glBindTexture(GL_TEXTURE_2D, m_texture));
      for (int i = 0; i < rectangle.height; ++i) {
        glCheck(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, i, rectangle.width, 1,
...
Image Texture::copyToImage() const {
  ...
  if (frameBuffer) {
    ...
    glCheck(glReadPixels(0, 0, m_size.x, m_size.y, GL_RGBA, GL_UNSIGNED_BYTE,
                         &pixels[0]));
...
////////////////////////////////////////////////////////////
void Texture::update(const Uint8 *pixels, unsigned int width,
  ...
  if (pixels && m_texture) {
    ...
    glCheck(glBindTexture(GL_TEXTURE_2D, m_texture));
    glCheck(glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, width, height, GL_RGBA,
...
void Texture::update(const Texture &texture, unsigned int x, unsigned int y) {
  ...
  if (GLEXT_framebuffer_object && GLEXT_framebuffer_blit) {
    ...
    if (!sourceFrameBuffer || !destFrameBuffer) {
      err() << "Cannot copy texture, failed to create a frame buffer object"
            << std::endl;
      return;
    }
    ...
    glCheck(GLEXT_glFramebufferTexture2D(GLEXT_GL_READ_FRAMEBUFFER,
                                         GLEXT_GL_COLOR_ATTACHMENT0,
                                         GL_TEXTURE_2D, texture.m_texture, 0));
    ...
    GLenum sourceStatus;
    glCheck(sourceStatus =
                GLEXT_glCheckFramebufferStatus(GLEXT_GL_READ_FRAMEBUFFER));

    GLenum destStatus;
    glCheck(destStatus =
                GLEXT_glCheckFramebufferStatus(GLEXT_GL_DRAW_FRAMEBUFFER));

    if ((sourceStatus == GLEXT_GL_FRAMEBUFFER_COMPLETE) &&
        (destStatus == GLEXT_GL_FRAMEBUFFER_COMPLETE)) {
      // Blit the texture contents from the source to the destination texture
      glCheck(GLEXT_glBlitFramebuffer(
          0, texture.m_pixelsFlipped ? texture.m_size.y : 0, texture.m_size.x,
          texture.m_pixelsFlipped
              ? 0
              : texture.m_size
                    .y, // Source rectangle, flip y if source is flipped
          x, y, x + texture.m_size.x,
          y + texture.m_size.y, // Destination rectangle
          GL_COLOR_BUFFER_BIT, GL_NEAREST));
    } else {
      err() << "Cannot copy texture, failed to link texture to frame buffer"
            << std::endl;
    }
...
void Texture::update(const Window &window, unsigned int x, unsigned int y) {
  ...
  if (m_texture && window.setActive(true)) {
    ...
    glCheck(glBindTexture(GL_TEXTURE_2D, m_texture));
    glCheck(glCopyTexSubImage2D(GL_TEXTURE_2D, 0, x, y, 0, 0,
...
void Texture::setSmooth(bool smooth) {
  if (smooth != m_isSmooth) {
    ...
    if (m_texture) {
      ...
      if (m_hasMipmap) {
        glCheck(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                                m_isSmooth ? GL_LINEAR_MIPMAP_LINEAR
...
////////////////////////////////////////////////////////////
bool Texture::isSmooth() const { return m_isSmooth; }

////////////////////////////////////////////////////////////
void Texture::setSrgb(bool sRgb) { m_sRgb = sRgb; }

////////////////////////////////////////////////////////////
bool Texture::isSrgb() const { return m_sRgb; }
...
////////////////////////////////////////////////////////////
bool Texture::isRepeated() const { return m_isRepeated; }
  ...
  if (texture && texture->m_texture) {
    ...
    if ((coordinateType == Pixels) || texture->m_pixelsFlipped) {
      ...
      if (texture->m_pixelsFlipped) {
        matrix[5] = -matrix[5];
        matrix[13] =
            static_cast<float>(texture->m_size.y) / texture->m_actualSize.y;
      }
      ...
      // Go back to model-view mode (meow::RenderTarget relies on it)
...
unsigned int Texture::getValidSize(unsigned int size) {
  if (GLEXT_texture_non_power_of_two) {
    ...
    // If hardware doesn't support NPOT textures, we calculate the nearest power
    // of two
