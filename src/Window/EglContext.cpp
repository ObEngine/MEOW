...
#include <meow/Window/WindowImpl.hpp>
#ifdef SFML_SYSTEM_ANDROID
#include <meow/System/Android/Activity.hpp>
#endif
...
EGLDisplay getInitializedDisplay() {
  ...
  meow::priv::ActivityStates *states = meow::priv::getActivity(NULL);
  meow::Lock lock(states->mutex);
...
EglContext::EglContext(EglContext *shared)
    : m_display(EGL_NO_DISPLAY), m_context(EGL_NO_CONTEXT),
      m_surface(EGL_NO_SURFACE), m_config(NULL) {
  ...
  m_config = getBestConfig(m_display, VideoMode::getDesktopMode().bitsPerPixel,
                           ContextSettings());
  updateSettings();

  // Note: The EGL specs say that attrib_list can be NULL when passed to
  // eglCreatePbufferSurface, but this is resulting in a segfault. Bug in
  // Android?
  EGLint attrib_list[] = {EGL_WIDTH, 1, EGL_HEIGHT, 1, EGL_NONE};

  eglCheck(m_surface =
               eglCreatePbufferSurface(m_display, m_config, attrib_list));
...
EglContext::~EglContext() {
  ...
  if (currentContext == m_context) {
    eglCheck(eglMakeCurrent(m_display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                            EGL_NO_CONTEXT));
  }
...
bool EglContext::makeCurrent(bool current) {
  ...
  if (current) {
    ...
    eglCheck(result = eglMakeCurrent(m_display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                                     EGL_NO_CONTEXT));
  }
...
void EglContext::createContext(EglContext *shared) {
  ...
  // Create EGL context
  eglCheck(m_context =
               eglCreateContext(m_display, m_config, toShared, contextVersion));
}
...
void EglContext::destroySurface() {
  // Ensure that this context is no longer active since our surface is going to
  // be destroyed
  setActive(false);
...
EGLConfig EglContext::getBestConfig(EGLDisplay display,
  ...
  const EGLint attributes[] = {
      ...
      EGL_SAMPLE_BUFFERS,
      static_cast<EGLint>(settings.antialiasingLevel ? 1 : 0),
      EGL_SAMPLES,
  ...
  // TODO: This should check EGL_CONFORMANT and pick the first conformant
  // configuration.
...
void EglContext::updateSettings() {
  ...
  eglCheck(result =
               eglGetConfigAttrib(m_display, m_config, EGL_DEPTH_SIZE, &tmp));
  ...
  eglCheck(result =
               eglGetConfigAttrib(m_display, m_config, EGL_STENCIL_SIZE, &tmp));
...
#ifdef SFML_SYSTEM_LINUX
////////////////////////////////////////////////////////////
XVisualInfo EglContext::selectBestVisual(::Display *XDisplay,
                                         unsigned int bitsPerPixel,
                                         const ContextSettings &settings) {
  EglContextImpl::ensureInit();
  ...
  eglCheck(eglGetConfigAttrib(display, config, EGL_NATIVE_VISUAL_ID,
                              &nativeVisualId));

  if (nativeVisualId == 0) {
    // Should never happen...
    err() << "No EGL visual found. You should check your graphics driver"
          << std::endl;
  ...
  availableVisuals =
      XGetVisualInfo(XDisplay, VisualIDMask, &vTemplate, &visualCount);

  if (visualCount == 0) {
    // Can't happen...
    err() << "No X11 visual found. Bug in your EGL implementation ?"
          << std::endl;
