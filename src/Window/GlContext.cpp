...
#include <meow/System/Lock.hpp>
#include <meow/System/Mutex.hpp>
#include <meow/System/ThreadLocalPtr.hpp>
#include <meow/Window/Context.hpp>
#include <meow/Window/EglContext.hpp>
#include <meow/Window/GlContext.hpp>
#include <set>
#include <string>
#include <utility>
#include <vector>
...
typedef meow::priv::EglContext ContextType;

#else

#include <meow/Window/Win32/WglContext.hpp>
typedef meow::priv::WglContext ContextType;
...
#include <meow/Window/Unix/GlxContext.hpp>
typedef meow::priv::GlxContext ContextType;

#endif

#elif defined(SFML_SYSTEM_MACOS)

#include <meow/Window/OSX/SFContext.hpp>
typedef meow::priv::SFContext ContextType;

#elif defined(SFML_SYSTEM_IOS)

#include <meow/Window/iOS/EaglContext.hpp>
typedef meow::priv::EaglContext ContextType;
...
meow::ThreadLocalPtr<meow::priv::GlContext> currentContext(NULL);

// The hidden, inactive context that will be shared with all other contexts
ContextType *sharedContext = NULL;

// Unique identifier, used for identifying contexts when managing unshareable
// OpenGL resources
meow::Uint64 id = 1; // start at 1, zero is "no context"
...
typedef std::set<std::pair<meow::ContextDestroyCallback, void *>>
    ContextDestroyCallbacks;
...
struct TransientContext : private meow::NonCopyable {
  ////////////////////////////////////////////////////////////
  /// \brief Constructor
  ///
  ////////////////////////////////////////////////////////////
  TransientContext()
      : referenceCount(0), context(0), sharedContextLock(0),
        useSharedContext(false) {
    if (resourceCount == 0) {
      context = new meow::Context;
    } else if (!currentContext) {
      sharedContextLock = new meow::Lock(mutex);
  ...
  meow::Context *context;
  meow::Lock *sharedContextLock;
...
meow::ThreadLocalPtr<TransientContext> transientContext(NULL);
...
void loadExtensions() {
  ...
  glGetErrorFuncType glGetErrorFunc = reinterpret_cast<glGetErrorFuncType>(
      meow::priv::GlContext::getFunction("glGetError"));
  glGetIntegervFuncType glGetIntegervFunc =
      reinterpret_cast<glGetIntegervFuncType>(
          meow::priv::GlContext::getFunction("glGetIntegerv"));
  glGetStringFuncType glGetStringFunc = reinterpret_cast<glGetStringFuncType>(
      meow::priv::GlContext::getFunction("glGetString"));
  ...
  glGetStringiFuncType glGetStringiFunc =
      reinterpret_cast<glGetStringiFuncType>(
          meow::priv::GlContext::getFunction("glGetStringi"));

  if (glGetErrorFunc() == GL_INVALID_ENUM || !glGetStringiFunc) {
    // Try to load the < 3.0 way
    const char *extensionString =
        reinterpret_cast<const char *>(glGetStringFunc(GL_EXTENSIONS));
    ...
    if (numExtensions) {
      for (unsigned int i = 0; i < static_cast<unsigned int>(numExtensions);
           ++i) {
        const char *extensionString =
            reinterpret_cast<const char *>(glGetStringiFunc(GL_EXTENSIONS, i));
...
bool parseVersionString(const char *version, const char *prefix,
...
////////////////////////////////////////////////////////////
void GlContext::registerContextDestroyCallback(ContextDestroyCallback callback,
                                               void *arg) {
...
GlContext *GlContext::create() {
  ...
  // Make sure that there's an active context (context creation may need
  // extensions, and thus a valid context)
...
////////////////////////////////////////////////////////////
GlContext *GlContext::create(const ContextSettings &settings,
  ...
  // If resourceCount is 1 we know that we are inside meow::Context or
  // meow::Window Only in this situation we allow the user to indirectly
  ...
  if ((resourceCount == 1) &&
    ...
    ContextSettings sharedSettings(0, 0, 0, settings.majorVersion,
...
////////////////////////////////////////////////////////////
const ContextSettings &GlContext::getSettings() const { return m_settings; }
...
bool GlContext::setActive(bool active) {
  ...
  if (active) {
    if (this != currentContext) {
      ...
      // This context is already the active one on this thread, don't do
      // anything
      return true;
    }
  } else {
...
////////////////////////////////////////////////////////////
int GlContext::evaluateFormat(unsigned int bitsPerPixel,
                              const ContextSettings &settings, int colorBits,
                              int depthBits, int stencilBits, int antialiasing,
                              bool accelerated, bool sRgb) {
  ...
  int antialiasingDiff =
      static_cast<int>(settings.antialiasingLevel) - antialiasing;

  // Weight sub-scores so that better settings don't score equally as bad as
  // worse settings
  ...
  int score = std::abs(colorDiff) + std::abs(depthDiff) +
              std::abs(stencilDiff) + std::abs(antialiasingDiff);
...
void GlContext::cleanupUnsharedResources() {
  ...
  for (ContextDestroyCallbacks::iterator iter = contextDestroyCallbacks.begin();
       iter != contextDestroyCallbacks.end(); ++iter)
...
void GlContext::initialize(const ContextSettings &requestedSettings) {
  ...
  glGetIntegervFuncType glGetIntegervFunc =
      reinterpret_cast<glGetIntegervFuncType>(getFunction("glGetIntegerv"));
  glGetErrorFuncType glGetErrorFunc =
      reinterpret_cast<glGetErrorFuncType>(getFunction("glGetError"));
  glGetStringFuncType glGetStringFunc =
      reinterpret_cast<glGetStringFuncType>(getFunction("glGetString"));
  glEnableFuncType glEnableFunc =
      reinterpret_cast<glEnableFuncType>(getFunction("glEnable"));
  glIsEnabledFuncType glIsEnabledFunc =
      reinterpret_cast<glIsEnabledFuncType>(getFunction("glIsEnabled"));

  if (!glGetIntegervFunc || !glGetErrorFunc || !glGetStringFunc ||
      !glEnableFunc || !glIsEnabledFunc) {
  ...
  if (glGetErrorFunc() != GL_INVALID_ENUM) {
    ...
    const char *version =
        reinterpret_cast<const char *>(glGetStringFunc(GL_VERSION));
    if (version) {
      // OpenGL ES Common Lite profile: The beginning of the returned string is
      // "OpenGL ES-CL major.minor" OpenGL ES Common profile:      The beginning
      // of the returned string is "OpenGL ES-CM major.minor" OpenGL ES Full
      // profile:        The beginning of the returned string is "OpenGL ES
      // major.minor" Desktop OpenGL:                The beginning of the
      // returned string is "major.minor"

      using GlContextImpl::parseVersionString;

      if (!parseVersionString(version, "OpenGL ES-CL ", m_settings.majorVersion,
                              m_settings.minorVersion) &&
          !parseVersionString(version, "OpenGL ES-CM ", m_settings.majorVersion,
                              m_settings.minorVersion) &&
          !parseVersionString(version, "OpenGL ES ", m_settings.majorVersion,
                              m_settings.minorVersion) &&
          !parseVersionString(version, "", m_settings.majorVersion,
                              m_settings.minorVersion)) {
        err() << "Unable to parse OpenGL version string: \"" << version
  ...
  // If the context was created with wglCreateContext, it is guaranteed to be
  // compatibility. If a 3.0 context was created with
  // wglCreateContextAttribsARB, it is guaranteed to be compatibility. If a 3.1
  // context was created with wglCreateContextAttribsARB, the compatibility flag
  // is set only if ARB_compatibility is present
  // If a 3.2+ context was created with wglCreateContextAttribsARB, the
  // compatibility flag would have been set correctly already depending on
  // whether ARB_create_context_profile is supported.

  // If the user requests a 3.0 context, it will be a compatibility context
  // regardless of the requested profile. If the user requests a 3.1 context and
  // its creation was successful, the specification states that it will not be a
  // compatibility profile context regardless of the requested profile unless
  // ARB_compatibility is present.
      ...
      if (glGetStringiFunc) {
        ...
        for (unsigned int i = 0; i < static_cast<unsigned int>(numExtensions);
          ...
          const char *extensionString = reinterpret_cast<const char *>(
              glGetStringiFunc(GL_EXTENSIONS, i));

          if (std::strstr(extensionString, "GL_ARB_compatibility")) {
            m_settings.attributeFlags &=
                ~static_cast<Uint32>(ContextSettings::Core);
            break;
          }
        }
      }
...
void GlContext::checkSettings(const ContextSettings &requestedSettings) {
  // Perform checks to inform the user if they are getting a context they might
  // not have expected
  ...
  const char *vendorName =
      reinterpret_cast<const char *>(glGetStringFunc(GL_VENDOR));
  const char *rendererName =
      reinterpret_cast<const char *>(glGetStringFunc(GL_RENDERER));

  if (vendorName && rendererName) {
    if ((std::strcmp(vendorName, "Microsoft Corporation") == 0) &&
        (std::strcmp(rendererName, "GDI Generic") == 0)) {
      err() << "Warning: Detected \"Microsoft Corporation GDI Generic\" OpenGL "
               "implementation"
            << std::endl
  ...
  int requestedVersion =
      requestedSettings.majorVersion * 10 + requestedSettings.minorVersion;
  ...
  if ((m_settings.attributeFlags != requestedSettings.attributeFlags) ||
    ...
    err() << "Warning: The created OpenGL context does not fully meet the "
             "settings that were requested"
          << std::endl;
    err() << "Requested: version = " << requestedSettings.majorVersion << "."
          ...
          << std::endl;
    err() << "Created: version = " << m_settings.majorVersion << "."
          << m_settings.minorVersion
