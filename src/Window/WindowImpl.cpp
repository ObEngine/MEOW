...
#include <meow/Window/Win32/WindowImplWin32.hpp>
typedef meow::priv::WindowImplWin32 WindowImplType;
...
#include <meow/Window/Unix/WindowImplX11.hpp>
typedef meow::priv::WindowImplX11 WindowImplType;
...
#include <meow/Window/OSX/WindowImplCocoa.hpp>
typedef meow::priv::WindowImplCocoa WindowImplType;

#define SFML_VULKAN_IMPLEMENTATION_NOT_AVAILABLE

#elif defined(SFML_SYSTEM_IOS)

#include <meow/Window/iOS/WindowImplUIKit.hpp>
typedef meow::priv::WindowImplUIKit WindowImplType;

#define SFML_VULKAN_IMPLEMENTATION_NOT_AVAILABLE

#elif defined(SFML_SYSTEM_ANDROID)

#include <meow/Window/Android/WindowImplAndroid.hpp>
typedef meow::priv::WindowImplAndroid WindowImplType;
...
WindowImpl *WindowImpl::create(VideoMode mode, const String &title,
                               Uint32 style, const ContextSettings &settings) {
...
////////////////////////////////////////////////////////////
WindowImpl::WindowImpl() : m_joystickThreshold(0.1f) {
  // Get the initial joystick states
  JoystickManager::getInstance().update();
  for (unsigned int i = 0; i < Joystick::Count; ++i) {
    m_joystickStates[i] = JoystickManager::getInstance().getState(i);
    std::fill_n(m_previousAxes[i],
                static_cast<std::size_t>(Joystick::AxisCount), 0.f);
  }
...
bool WindowImpl::popEvent(Event &event, bool block) {
  // If the event queue is empty, let's first check if new events are available
  // from the OS
  if (m_events.empty()) {
...
////////////////////////////////////////////////////////////
void WindowImpl::pushEvent(const Event &event) { m_events.push(event); }
...
void WindowImpl::processJoystickEvents() {
  ...
  for (unsigned int i = 0; i < Joystick::Count; ++i) {
    ...
    if (previousState.connected ^ connected) {
      Event event;
      event.type =
          connected ? Event::JoystickConnected : Event::JoystickDisconnected;
...
////////////////////////////////////////////////////////////
bool WindowImpl::createVulkanSurface(const VkInstance &instance,
  ...
  return VulkanImplType::createVulkanSurface(instance, getSystemHandle(),
                                             surface, allocator);
